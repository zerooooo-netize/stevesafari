...
const MpesaPaymentWidget = ({
 ...
 return (
<div className="text-center py-6"><Loader2 size={32} className="animate-spin mx-auto text-safari-gold mb-3"/><p className="font-medium text-sm">Check your phone!</p><p className="text-xs text-muted-foreground mt-1">Enter your M-Pesa PIN when prompted.
</p></div>);
 }

 if (payStatus === "completed") {
 return (
<div className="text-center py-6"><CheckCircle2 size={32} className="mx-auto text-green-600 mb-3"/><p className="font-medium text-sm text-green-700">Payment Successful!</p><p className="text-xs text-muted-foreground mt-1">Your order has been placed.
</p></div>);
 }
...
<div className="space-y-3"><div><Label className="text-xs">Phone Number *</Label><Input
 ...
 placeholder="+254712345678" className="text-sm"/></div><div className="text-[11px] text-muted-foreground bg-muted/30 rounded p-2 flex items-start gap-1.5"><Shield size={12} className="text-safari-gold mt-0.5 shrink-0"/><span>Securely processed via M-Pesa (Kopo Kopo). Official receipt provided.
...
<p className="text-xs text-destructive text-center">Payment failed. Try again.</p>)}
 {payStatus === "timeout"&& (
<p className="text-xs text-yellow-600 text-center">Payment not confirmed. Check history.
</p>)}
</div>);
};
...
<div className="min-h-screen grid place-items-center bg-background"><Loader2 className="animate-spin text-safari-gold" size={28} /></div>);
 ...
 if (!service) {
 return (
<div className="min-h-screen bg-background"><Navbar /><main className="container pt-24 pb-16 text-center"><p className="text-muted-foreground">Service not found.</p><Link to="/services" className="text-safari-gold underline mt-4 inline-block">← Back to services
</Link></main></div>);
 }
 ...
 return (
<div className="min-h-screen bg-background"><TrustBar /><Navbar /><main className="pt-24 sm:pt-28 section-y page-x"><div className="container max-w-3xl"><Link
...
<section className="bg-card border border-border rounded-2xl p-6 sm:p-8 shadow-card mb-6"><h2 className="font-heading font-bold text-lg mb-4 flex items-center gap-2"><CheckCircle2 size={18} className="text-safari-gold"/>What' s included
</h2><ul className="space-y-2 text-sm">{[
 " Professional review by our specialists",
 "1 round of revisions included",
 "Delivery within 3–5 business days",
 "Final document delivered as PDF",
 ].map((item, i) =>(
<li key={i} className="flex items-start gap-2"><CheckCircle2 size={14} className="text-green-600 shrink-0 mt-0.5"/><span>{item}</span></li>))}
</ul></section>{/* Pricing options */}
<section className="bg-card border border-border rounded-2xl p-6 sm:p-8 shadow-card mb-6"><h2 className="font-heading font-bold text-lg mb-3">Payment options</h2><div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><button
...
<div className="bg-card border border-border rounded-2xl p-6 shadow-elevated"><div className="flex justify-between items-center mb-4"><h3 className="font-heading font-bold text-lg">{orderStep === "details"? " Tell us what you need": "Complete Payment"}
</h3><button
 onClick={() =>setShowOrder(false)}
 className="p-1 hover:bg-muted rounded"><X size={20} /></button></div>{orderStep === "details"? (
<div className="space-y-3"><div><Label className="text-xs">Details *</Label><Textarea
 value={details}
 onChange={(e) =>setDetails(e.target.value.slice(0, 1000))}
 placeholder=" Describe your needs, target country, role, deadline, etc." className="text-sm min-h-[120px]"/><p className="text-[10px] text-muted-foreground mt-1">{details.length}/1000
</p></div><div><Label className="text-xs">Upload existing file (optional)</Label><label className="flex items-center gap-2 border border-dashed border-border rounded-lg p-3 text-sm cursor-pointer hover:bg-muted/30"><Upload size={16} className="text-muted-foreground"/><span className="truncate flex-1">{file?.name || " Choose file (max 10MB)"}
</span><input
 type="file" className="hidden" onChange={(e) =>setFile(e.target.files?.[0] || null)}
 /></label></div><div className="flex gap-2"><Button
 variant="outline" className="flex-1" onClick={() =>setShowOrder(false)}
 >Cancel
</Button><Button
 ...
 >Continue to Payment<ArrowLeft className="rotate-180 ml-1" size={14} /></Button></div></div>) : (
<div className="space-y-3">{/* Order summary */}
...
<span className="block text-[10px] text-muted-foreground font-normal">Balance {format(halfPrice, "KES")} due before delivery</span>)}
 ...
 className="text-xs text-muted-foreground hover:underline w-full text-center">← Back to edit details
</button><p className="text-[11px] text-muted-foreground text-center">You' ll receive an official receipt via email after payment.
</p></div>)}
</div>)}

 {/* Anti-scam */}
<div className="mt-6 flex items-start gap-2 bg-yellow-50 border border-yellow-200 rounded-xl p-4"><AlertCircle className="text-yellow-700 shrink-0 mt-0.5" size={16} /><p className="text-xs text-yellow-900">All payments happen<strong>only through this platform</strong>via M-Pesa. We will
