  ...
  if (payStatus === "completed") {
    return (
      <div className="text-center py-6">
        ...
        <p className="text-xs text-muted-foreground mt-1">Your order has been placed.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="text-[11px] text-muted-foreground bg-muted/30 rounded p-2 flex items-start gap-1.5">
        ...
        <span>Securely processed via M-Pesa (Kopo Kopo). Official receipt provided.</span>
      </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8 sm:mb-12">
            ...
            <p className="text-muted-foreground mt-2 text-sm sm:text-base">
              Professional document preparation. Pay securely via M-Pesa.
            </p>
          </div>
              ...
              {services.map((service, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="font-heading font-bold text-safari-gold text-lg mb-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" className="flex-1 h-10" onClick={() => handleOrderClick(service)}>
            ...
            <div className="fixed inset-0 bg-foreground/50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
              <div className="bg-card rounded-t-2xl sm:rounded-2xl border border-border p-5 sm:p-6 w-full sm:max-w-md shadow-elevated max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <button onClick={() => setSelectedService(null)} className="p-1 hover:bg-muted rounded">
                    <X size={20} />
                  </button>
                </div>
                <p className="text-sm text-muted-foreground mb-4">
                  💰 Price: <span className="font-semibold text-safari-gold">{selectedService.currency} {Number(selectedService.price).toLocaleString()}</span>
                </p>

                {orderStep === "details" ? (
                  <>
                    <Label className="text-xs">What do you need? (Details)</Label>
                    <Textarea
                      value={details}
                      onChange={e => setDetails(e.target.value)}
                      rows={3}
                      placeholder="e.g. Update my CV for warehouse jobs in Toronto"
                    ...
                    <p className="text-xs text-muted-foreground mb-3">
                      After payment, your order will be processed by our team.
                    </p>
                    <div className="flex gap-2">
                      <Button
                      ...
                      >
                        Proceed to Payment <ArrowRight size={14} className="ml-1" />
                      </Button>
                      <Button variant="outline" onClick={() => setSelectedService(null)} className="h-11">
                        Cancel
                      </Button>
                    </div>
                  </>
                  ...
                  <>
                    ...
                    <button
                      ...
                      className="text-xs text-muted-foreground hover:underline mt-3 w-full text-center"
                    >
