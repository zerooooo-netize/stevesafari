...
const AuthPage = () => {
          ...
          <div className="bg-card border border-border rounded-2xl p-8 shadow-lg">
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                Already verified? <button onClick={() => setMode("login")} className="text-safari-gold hover:underline">Sign in</button>
              </p>
            </div>
          </div>
        </div>
  ...
  // Forgot-password "link sent" state
  if (resetSent) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="w-full max-w-md text-center">
          <div className="bg-card border border-border rounded-2xl p-8 shadow-lg">
            <div className="w-16 h-16 rounded-full bg-safari-gold/15 mx-auto flex items-center justify-center mb-4">
              <Mail className="text-safari-gold" size={28} />
            </div>
            ...
            <Button className="w-full" onClick={() => { setResetSent(false); setMode("login"); }}>
              Back to sign in
            </Button>
          </div>
        </div>
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="font-heading text-3xl font-bold text-foreground flex items-center justify-center gap-2">
            {isLogin ? "Welcome Back" : isSignup ? "Start Your Journey" : "Reset Password"}
            {!isForgot && <Briefcase size={24} className="text-safari-gold" />}
          </h1>
        ...
        <div className="grid grid-cols-2 mb-5 bg-muted/60 p-1 rounded-xl text-sm">
          <button
            type="button"
            onClick={() => setMode("login")}
            className={`h-9 rounded-lg font-medium transition ${isLogin || isForgot ? "bg-card shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"}`}
          >
            Sign in
          </button>
          <button
            ...
            className={`h-9 rounded-lg font-medium transition ${isSignup ? "bg-card shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"}`}
          >
            Create account
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-card/80 backdrop-blur-sm border border-border rounded-2xl p-6 shadow-xl space-y-5">
          {isSignup && (
            <div className="space-y-1.5">
              ...
              <Input id="fullName" value={fullName} onChange={(e) => setFullName(e.target.value)}
                placeholder="John Kamau" className="h-11" required autoComplete="name" />
            </div>
          )}
          ...
          <div className="space-y-1.5">
            ...
            <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)}
              placeholder="you@example.com" className="h-11" required autoComplete="email" />
          </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <Input id="password" type={showPassword ? "text" : "password"} value={password}
                  ...
                  className="h-11 pr-10" required minLength={6}
          ...
          {isSignup && (
            <div className="space-y-1.5">
              <Label htmlFor="referralCode" className="text-sm font-medium flex items-center gap-1">
                <Tag size={14} /> Referral Code (Optional)
              </Label>
              <Input id="referralCode" value={referralCode}
                onChange={(e) => setReferralCode(e.target.value.toUpperCase())}
                placeholder="e.g. AB12CD34" maxLength={12} className="h-11 uppercase" />
              {refFromUrl && (
                <p className="text-xs text-safari-gold flex items-center gap-1 mt-1">
                  <CheckCircle2 size={12} /> You were referred by code {refFromUrl}
                </p>
              )}
            </div>
          )}
          ...
          {isLogin && (
            <div className="text-right">
              <button type="button" onClick={() => setMode("forgot")}
                className="text-xs text-safari-gold hover:underline">
                Forgot password?
              </button>
            </div>
          )}
        ...
        <div className="flex items-center justify-center gap-5 mt-6 text-xs text-muted-foreground">
          <span className="inline-flex items-center gap-1"><ShieldCheck size={13} className="text-safari-gold" /> Secure</span>
          <span className="inline-flex items-center gap-1"><CheckCircle2 size={13} className="text-safari-gold" /> Trusted Agency</span>
          <span className="inline-flex items-center gap-1"><Users size={13} className="text-safari-gold" /> 500+ Placed</span>
        </div>
      </div>
    </div>
  );
