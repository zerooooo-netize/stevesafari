    ...
    >
      <div className="space-y-5 text-center">
        <div className="mx-auto w-24 h-24 rounded-full bg-primary/10 flex items-center justify-center">
          {isJobs ? <Plane size={48} className="text-primary" /> : <PartyPopper size={48} className="text-primary" />}
        </div>
        ...
        <div className="bg-muted/40 rounded-xl p-4 text-sm text-left space-y-2">
          ...
          <ul className="space-y-1 text-muted-foreground">
            {isJobs ? (
              <>
                <li>✈️ Final travel briefing by email/SMS</li>
                <li>📋 Pre-departure checklist sent 7 days before flight</li>
                <li>🏠 Accommodation confirmation upon arrival</li>
              </>
            ) : (
              <>
                <li>📄 Document processing typically takes 3–7 days</li>
                <li>📧 Email notification when ready</li>
                <li>⬇️ Download from your dashboard once complete</li>
              </>
            )}
          </ul>
        </div>
